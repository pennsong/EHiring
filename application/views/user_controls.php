<div id="header-wrapper">
	...
	if ($this->session->userdata('user') && $this->session->userdata('type'))
	{
		if ($this->session->userdata('type') == 'hunter')
		{
			...
			if ($this->db->count_all('T_hunter') > 0)
			{
				$log_in = TRUE;
			}
		}
		else if ($this->session->userdata('type') == 'hr')
		{
			...
			if ($this->db->count_all('T_Enterprise_user') > 0)
			{
				$log_in = TRUE;
			}
		}
	...
	<h1 class="span-4 prepend-2">
		<a href="#">E-hiring</a>
	</h1>
	<p class="span-4" style="margin-top: 15px; color: #FFFFFF">&nbsp;&nbsp;&nbsp;&nbsp;综合招聘信息平台</p>
